-- Price Program v4 (on top of the global price ladder from 202607020004):
-- 1. Two-month carry window. Reaching a quantity tier in ANY single calendar
--    month now grants that tier's discount for the NEXT TWO months, not just
--    one. Implemented as: this month's pricing floor ("ยศ") =
...
-- The tier itself is still chosen from the GLOBAL price_tiers ladder at
-- greatest(total pieces in the order, effective floor) — unchanged from 20260702_0004.
...
-- Reproduces the global-ladder create_order from 202607020004 (one discount
-- ladder, tier chosen from total pieces in the order or the floor) and only
-- changes how the pricing floor is derived.
