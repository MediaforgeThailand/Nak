...
**ขั้นที่ 2 — เลือกวิธีตามแต่ละบัญชี:**
- **บัญชีที่ไม่เคยมีออเดอร์/สลิปเลย** (orders = 0 และ payments = 0): ลบทิ้งได้สะอาดที่สุดจาก Supabase → **Authentication → Users** → กดลบ user (โปรไฟล์จะถูกลบตามอัตโนมัติ) — **ห้ามลบบัญชีที่ `is_owner = true`**
- **บัญชีที่เคยมีออเดอร์/สลิป หรือเคยเป็นแอดมิน** (เช่น `admin@admin.com`): **อย่าลบ** เพราะข้อมูลนี้ผูกกับประวัติออเดอร์/บัญชี ให้ **ระงับ** แทน (ระงับแล้วล็อกอินไม่ได้และไม่โผล่ในหน้าลูกค้า) — รัน SQL นี้ (แก้อีเมลตามที่เลือกจากขั้นที่ 1):
...
where email = any (array['admin@admin.com', 'taksinkubpom@gmail.com'])  -- << แก้รายการตามจริง
  and is_owner = false;   -- กันเผลอระงับเจ้าของ
```
> ห้ามใส่บัญชีเจ้าของจริงในรายการนี้ เงื่อนไข `is_owner = false` เป็นกันชนไว้อีกชั้น ถ้าเผลอใส่ก็จะไม่ถูกระงับ
