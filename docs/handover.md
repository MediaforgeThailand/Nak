...
- บัญชีทดสอบอื่น ๆ ที่เหลือ: ลบถาวรจากในระบบได้เลย (ต้องล็อกอินด้วย**บัญชีเจ้าของ**) — หน้า `/admin/users` การ์ดทีมงาน → "ลบบัญชีถาวร", หรือหน้าลูกค้ารายคน (`/admin/customers/<id>`) → "ลบบัญชีนี้" (ปุ่มมีเฉพาะเจ้าของ กดแล้วยืนยันอีกชั้น) — ลบถาวร ออเดอร์/ประวัติของบัญชีนั้นหายไปด้วย เหมาะกับบัญชีทดสอบเท่านั้น
- **บัญชี demo `admin@admin.com`**: เป็นเจ้าของปัจจุบัน (`is_owner`) — ต้อง**ตั้งเจ้าของตัวจริง + โอนสิทธิ์**ก่อน (ข้อ 4) แล้วจึงลบ/ระงับได้ (ลบบัญชี `is_owner` ไม่ได้จนกว่าจะถอนสิทธิ์เจ้าของออกก่อน)
- ถ้าอยากแค่ **ระงับ** (ปิดใช้ชั่วคราว กู้คืนได้ ไม่ลบประวัติ) แทนการลบ: `/admin/users` → "ระงับ"
