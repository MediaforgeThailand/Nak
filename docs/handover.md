...
- **ลบบัญชีทีมงาน**: แอดมินคนไหนก็ลบได้ — `/admin/users` การ์ดทีมงาน → "ลบบัญชีนี้ออกจากทีม" (ยืนยันอีกชั้น) — บัญชีถูกลบ ออเดอร์/สลิปของลูกค้ายังอยู่ครบ (แค่ไม่ผูกชื่อผู้ทำรายการ)
- **ลบบัญชีลูกค้า**: เฉพาะ**เจ้าของร้าน** — `/admin/customers/<id>` → "ลบบัญชีนี้" — เพราะลบแล้วประวัติการซื้อของลูกค้ารายนั้นหายไปด้วย (กันเผลอลบจนรายงานเพี้ยน)
...
- **ราคาขาย vs ราคาต้นทุน**: `/admin/products` เพิ่ม/แก้สินค้ามี 2 ช่อง — **ราคาขาย** (ลูกค้าเห็น) และ **ราคาต้นทุน** (ทีมงานเห็นเท่านั้น ลูกค้าไม่เห็นเด็ดขาด เก็บแยกตาราง `product_costs` + RLS); ลิสต์สินค้าฝั่งแอดมินโชว์ "ทุน ฿X" ให้ทีมงานดูกำไร
...
- **ยกเลิกออเดอร์หลังอนุมัติ** (ก่อนส่งของ): แท็บ "จัดสินค้า" → "ยกเลิกออเดอร์นี้" → ระบบคืนสต็อก + คืนยอดหนี้อัตโนมัติ พร้อมบันทึกใน ledger (เก็บออเดอร์ไว้เป็นสถานะ "ยกเลิก")
- **ลบออเดอร์ที่คีย์ผิด** (แอดมินเท่านั้น, ก่อนส่งของ): ในการ์ดออเดอร์ (รออนุมัติ / จัดสินค้า / จัดส่ง) → "ลบออเดอร์นี้ทิ้ง (คีย์ผิด)" → ลบถาวรไม่เหลือประวัติ + คืนสต็อก (และคืนยอดหนี้ถ้าอนุมัติแล้ว) — ต่างจาก "ยกเลิก" ที่เก็บบันทึกไว้ · ออเดอร์ที่ส่งของแล้วลบไม่ได้
