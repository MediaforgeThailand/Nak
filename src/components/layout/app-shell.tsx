...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-flow-col auto-cols-fr border-t border-border bg-surface md:hidden">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            prefetch={false}
            className={clsx(
              "flex min-h-16 min-w-0 flex-col items-center justify-center gap-1 px-1 text-[11px] font-medium leading-tight text-muted",
            )}
          >
