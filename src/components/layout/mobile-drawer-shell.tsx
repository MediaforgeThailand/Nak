  ...
  return (
    <div className="md:hidden">
      <button
        type="button"
        className={clsx(
          "mobile-drawer-button motion-surface fixed bottom-4 left-4 right-4 z-30 mx-auto inline-flex min-h-11 w-fit items-center justify-center gap-2 rounded-full border border-white/70 bg-white/88 px-4 text-sm font-semibold text-foreground shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_14px_34px_rgba(31,65,58,0.2)] backdrop-blur-2xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent",
        ...
        <aside className="mobile-drawer-panel absolute bottom-0 left-0 top-0 grid w-[min(86vw,340px)] grid-rows-[auto_1fr] overflow-hidden border-r border-white/70 bg-white/92 shadow-[0_24px_80px_rgba(16,32,29,0.28)] backdrop-blur-2xl">
          <div className="flex items-start justify-between gap-3 border-b border-border p-3">
            <div className="min-w-0">
              ...
              <h2 className="mt-1 text-base font-semibold">เมนูหลังบ้าน</h2>
              <p className="mt-1 break-words text-xs text-muted">{profileEmail}</p>
            </div>
            <button
              type="button"
              className="motion-surface grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/70 bg-white/78 text-muted shadow-[inset_0_1px_0_rgba(255,255,255,0.86),0_10px_24px_rgba(31,65,58,0.1)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
              aria-label="ปิดเมนูหลังบ้าน"
              onClick={() => setOpenPathname(null)}
            >
              <X className="h-4 w-4" />
          ...
          <nav
            ...
            className="grid content-start gap-1.5 overflow-y-auto p-3"
            onClick={(event) => {
